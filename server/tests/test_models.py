...
USER_CANNOT_FOLLOW_USER = {  # user has already followed him
    ...
    "result": {
        "message": ERROR_MESSAGE, "http_status_code": BAD_REQUEST_STATUS_CODE
    }
}
...
class TestHandleEndpoint:
    ...
    @staticmethod
    @pytest_mark.asyncio
    async def test_dislike_tweet_by_id(
            init_test_data_for_db: None) -> None:
    ...
    @staticmethod
    @pytest_mark.asyncio
    async def test_get_full_tweet_feed(
            init_test_data_for_db: None) -> None:
    ...
    async def test_get_user_profile_details(
            init_test_data_for_db: None) -> None:
        user_id = (
            CORRECT_GET_USER_PROFILE_RESPONSE)["user_profile"]["user"]["id"]
        user_profile, http_status_code = \
            await HandleEndpoint.get_user_profile_details(user_id)
        assert (user_profile ==
                CORRECT_GET_USER_PROFILE_RESPONSE["user_profile"])
        assert (http_status_code ==
    ...
    async def test_follow_other_user(init_test_data_for_db: None) -> None:
        ...
        assert http_status_code == expected_result["http_status_code"]
        assert (message.get("result", None) ==
