...
class TestTableUserMethods:
    ...
    async def test_is_existed_user_name(init_test_data_for_db: None) -> None:
        for i_exist_user in EXIST_USERS:
            assert (
                await User.is_existed_user_name(i_exist_user["name"]) is True
            )
        for i_unexist_user in UNEXIST_USERS:
            assert (
                await User.is_existed_user_name(i_unexist_user["name"])
                is False
            )
    ...
    async def test_get_user_id_by_name(init_test_data_for_db: None) -> None:
        for i_exist_user in EXIST_USERS:
            assert (
                await User.get_user_id_by_name(i_exist_user["name"])
                == i_exist_user["id"]
            )
        for i_unexist_user in UNEXIST_USERS:
            assert (
                await User.get_user_id_by_name(i_unexist_user["name"]) is None
            )
